...
#else
			if (check == false) {
				MainCScene::CreateShaderResourceView(pd3dDevice, this, nIndex);
			}
			else if (check == true) {
...
void CGameObject::SetLook(XMFLOAT3 pos)
{
	XMFLOAT4X4 xmf4x4View = Matrix4x4::LookAtLH(GetPosition(), pos, XMFLOAT3(0.0f, 1.0f, 0.0f));
...
{
	if (m_bActive) CGameObject::Render(pd3dCommandList, pCamera);
}
...
CBulletObject::CBulletObject(float fEffectiveRange, ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, int check) : CGameObject(1)
{
	m_fBulletEffectiveRange = fEffectiveRange;

	CCubeMeshDiffused* pBulletMesh = new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 1.1f, 1.1f, 1.1f);
	SetMesh(pBulletMesh);

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CTexture* pBulletTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1);
	pBulletTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/stone.dds", RESOURCE_TEXTURE2D, 0);
	...
	if (check == 1) MainCScene::CreateShaderResourceViews(pd3dDevice, pBulletTexture, 0, 7);
	if (check == 2) CScene::CreateShaderResourceViews(pd3dDevice, pBulletTexture, 0, 7);
...
void CBulletObject::Animate(float fElapsedTime)
{
	...
	// 바운딩 박스를 현재 위치에 맞게 설정
	m_BulletBoundingBox = BoundingOrientedBox(
		XMFLOAT3(xmf3Position.x, xmf3Position.y, xmf3Position.z),
	...
	XMMATRIX xmRotate = XMMatrixRotationRollPitchYaw(XMConvertToRadians(0.0f), XMConvertToRadians(1.0f * fElapsedTime), XMConvertToRadians(0.0f));
	XMStoreFloat4x4(&m_xmf4x4Transform, xmRotate * XMLoadFloat4x4(&m_xmf4x4Transform));
	...
	// 총알이 유효 범위를 초과하거나 잠금 시간이 경과한 경우 리셋
...
CTowerObject::CTowerObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1)
{
	CCubeMeshDiffused* pTowerMesh = new CCubeMeshDiffused(pd3dDevice, pd3dCommandList, 100.1f, 100.1f, 100.1f);
	SetMesh(pTowerMesh);
	
	// 랜덤 엔진과 분포 생성
	std::random_device rd;
	std::mt19937 gen(rd());
	std::uniform_real_distribution<float> disX(500.0f, 1500.0f);
	std::uniform_real_distribution<float> disZ(500.0f, 1500.0f);

	// x, z 좌표를 랜덤하게 설정하고 y는 일정하게 유지
	float x = disX(gen);
	float z = disZ(gen);
	SetPosition(x, 500.0f, z);

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CTexture* pTowerTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1);
	pTowerTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/rocks2.dds", RESOURCE_TEXTURE2D, 0);

	CTowerShader* pTowerShader = new CTowerShader();
	pTowerShader->CreateShader(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
	pTowerShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CScene::CreateShaderResourceViews(pd3dDevice, pTowerTexture, 0, 8);

	CMaterial* pTowerMaterial = new CMaterial();
	pTowerMaterial->SetTexture(pTowerTexture);
	pTowerMaterial->SetShader(pTowerShader);

	SetMaterial(0, pTowerMaterial);
}

CTowerObject::~CTowerObject()
{
}

void CTowerObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	UpdateShaderVariables(pd3dCommandList);
	if (m_pShader) m_pShader->Render(pd3dCommandList, pCamera);
	if (m_pMesh) m_pMesh->Render(pd3dCommandList, 0);
