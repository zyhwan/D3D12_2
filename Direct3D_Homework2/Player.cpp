...
CAirplanePlayer::CAirplanePlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, int Check)
{
	if (Check == 1 || Check == 3) {
	...
	m_bShader = new CBulletShader();
	m_bShader->CreateShader(pd3dDevice, pd3dGraphicsRootSignature);
	...
	{
		m_ppBullets[i] = new CBulletObject(150.0f, pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, check);
		m_ppBullets[i]->SetMovingSpeed(120.0f);
		m_ppBullets[i]->SetActive(false);
		m_ppBullets[i]->m_BulletBoundingBox = BoundingOrientedBox(
			XMFLOAT3(1000.f, 500.f, 1000.f),
